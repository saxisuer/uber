...
<html>
  ...
  <body id="body">
    ...
    <!-- 表格顶部工具栏 -->
	<div id="toolbar">
	    <a href="javascript:void(0)"  id="writeDeliverMark" class="easyui-linkbutton" iconCls="icon-add" plain="true">发货</a>
...
$(function(){
	...
	gridDom.datagrid({
		...
		width:$('#body').width(),
		idField:'id',
		url:'<%=path%>/applicant/loadDeliveryData',   
		singleSelect:false, 
		selectOnCheck: true,
		checkOnSelect: true,
		nowrap:true,
	...
	//发货
	$("#writeDeliverMark").on("click", function(){
		...
		console.log(idStr);
		$.ajax({
			url: 'applicant/writeDeliverMark',
			data: {'userAccs':idStr},
			success: function(data){
				console.log(data);
				$('#applicant').datagrid('reload');
			}
		});
...
function search(){
	$('#applicant').datagrid('load',{ param_mobile: $.trim($('#param_mobile').val()),
