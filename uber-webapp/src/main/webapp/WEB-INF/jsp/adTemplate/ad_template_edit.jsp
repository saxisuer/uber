...
<form action="${pageContext.request.contextPath}/adTemplate/save" method="post">
    ...
    <input id="adFileIdList" value='${adTemplate.adFileString}' type="hidden"/>
    <div class="user-form">
        <h2>修改广告模板</h2>
        <div class="yui3-g">
            <div class="yui3-u-1-2">
                <div class="formgourp">
                    <label>广告组名：</label>
                    <div class="textbox">
                        <input id="templateName" class="textbox-text validatebox-text textbox-prompt" value="${adTemplate.templateName}"
                               name="templateName"/>
                    </div>
                </div>
                <div class="formgourp">
                    <label>广告组描述：</label>
                    <div class="textbox">
                        <input id="templateDesc" class="textbox-text validatebox-text textbox-prompt" value="${adTemplate.templateDesc}"
                               name="templateDesc"/>
                    </div>
                </div>
                <div class="formgourp">
...
<script type="text/javascript">
    ...
    function initDiv() {
        var fileListArray = eval('(' + $('#adFileIdList').val() + ')');
        var ul = $("<ul></ul>").addClass("o-display-role-list");
        var fileList = [];
        console.log(fileListArray);
        $.each(fileListArray, function (i, v) {
            fileList.push(v.fileId);
            $("<li>" + v.fileTitle + "</li>").attr("fileId", v.fileId).prepend($("<span>x</span>").addClass("o-display-remove").bind("click",
                    function (e) {
                        $(this).parent().remove();
